...
- **Native keyboard control** for navigation without additional modules (enable with `enableKeyboardControl: true`).
- Integration with [MMM-KeyBindings](https://github.com/shbatm/MMM-KeyBindings) for advanced keyboard and bluetooth remote navigation.
...
| `ignoreModules`                                                                                                                                                                   | A list of module names whom should not be considered as part of the carousel. For example, the `alert` module should be able to display a notification at any time, by ignoring it we can prevent the plugin from hiding any notifications. NOTE: is only used in `global` and `slides` modes. Ignored modules in `slides` mode are shown on every slide.<br><br>**Multiple instances of the same module:** If you have multiple instances of the same module and want to ignore only specific ones, add a `carouselId` to the module's config and use that ID in `ignoreModules` instead of the module name.<br><br>This value is **OPTIONAL**<br>Possible values: String array<br>Default value: `[]`                                                                                                                          |
...
| `enableKeyboardControl`                                                                                                                                                           | A `boolean` to enable native keyboard control without requiring MMM-KeyBindings module. **Note:** Only works in `global` or `slides` mode, not in `positional` mode.<br><br>**Keyboard shortcuts:**<br>• **Arrow Right** ― Next slide<br>• **Arrow Left** ― Previous slide<br>• **Arrow Down** ― Pause/Resume rotation<br>• **Home** or **0** ― Jump to home slide<br>• **1-9** ― Jump to slide 1-9 (key "1" = first slide, "2" = second slide, etc.)<br><br>This value is **OPTIONAL**<br>Default value: `false`                                                                                                                                                                                                                                                                                                                |
...
```javascript
    {
      ...
      config: {
        ...
        top_right: {
          enabled: true,
          ignoreModules: ["currentweather"]  // Ignore by module name or carouselId
        }
      }
    },
```
...
**Using `carouselId`:** When you have multiple instances of the same module, set a unique `carouselId` in each module's config to identify them individually on different slides or in `ignoreModules`.
...
| `CAROUSEL_TOGGLE_AUTO` | Switch between automatic rotation and manual mode. When manual mode is enabled the carousel stops scheduling automatic transitions until this notification is sent again.                                                                                                                                                    |
...
- `node --run demo:global` - Start MagicMirror with the global demo configuration.
